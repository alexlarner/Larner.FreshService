function New-FSUserCreationRequest {
	...
	begin {
		...
		if ($IncludePasswordResetTask.IsPresent) { $TaskTitles += 'Reset user''s AD password and send it to their manager via encrypted email' }
	}
	process {
		...
		$UpdateTicketSplat = @{
			...
			ResponderId = $HelpdeskAutomationAgentID
		...
		if ($PassThru.IsPresent) { Write-Output $NewSR }
	}
}
