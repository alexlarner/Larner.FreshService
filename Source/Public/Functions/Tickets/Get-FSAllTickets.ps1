function Get-FSAllTickets {
	...
	if ($UpdatedSince) {
		$PSBoundParameters.UpdatedSince = if ($UpdatedSince.TimeOfDay -eq '00:00:00') {
			Get-Date $UpdatedSince -UFormat %Y-%m-%d
		} else {
			Get-Date $UpdatedSince -UFormat %Y-%m-%dT%TZ%Z
		}
	}

	$Body = ConvertTo-FSNames -FrontEndParams $PSBoundParameters -ExcludePair $APIDefaultValues -PreserveStringValue 'Type', 'UpdatedSince'
	...
	Invoke-PSFProtectedCommand -Action 'Getting all tickets with this filter' -Target $Body -ScriptBlock {
		$Result = Invoke-FSAPI -URISuffix '/tickets' -Body $Body
	} -Continue
