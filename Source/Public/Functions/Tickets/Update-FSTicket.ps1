...
#>
	...
	begin {
		...
		$ConvertSplat = @{
			...
			PreserveStringValue = '*'
			ExcludeKey = 'ID', 'DeleteExistingTags'
		}

		#-ExcludePair $Defaults
		$Body = ConvertTo-FSNames @ConvertSplat
	}
		...
		foreach ($Ticket in $ID) {
			...
			if ($DeleteExistingTags) {
				...
				if ($UniqueBody.ContainsKey('Tags')) {
					[string[]]$ExistingTags = (Get-FSTicket -ID $Ticket.ID -Include Tags).Tags
					if ($ExistingTags.Count -gt 0) { $UniqueBody.Tags = @($UniqueBody.Tags + $ExistingTags | Select-Object -Unique) }
				}
			}
			Invoke-PSFProtectedCommand -Action "Update ticket $Ticket" -Target $Ticket -ScriptBlock {
				$Result = Invoke-FSAPI -URISuffix "/tickets/$($Ticket.ID)" -Body $UniqueBody -Method Put
			} -Continue

			if ($PassThru) {
				$UpdatedTicket = [Ticket]::New($Result.Ticket)
				if (-Not $DisablePropertyExpansion) {
					$UpdatedTicket.UpdateCustomClassProperties()
				}
				Write-Output $UpdatedTicket
			}
		}
	}
}
