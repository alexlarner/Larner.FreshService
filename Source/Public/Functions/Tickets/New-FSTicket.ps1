function New-FSTicket {
	...
	param(
		...
		This is mandatory due to company policy, not manufacturer's policy.
		#>
		[Parameter(Mandatory)]
		[long]$GroupId,
	...
	process {
		foreach ($Sub in $Subject) {
			$Splat.Body.Subject = $Sub
			Invoke-PSFProtectedCommand -Action "Creating ticket for $Sub" -ScriptBlock {
			...
			Write-PSFMessage "Created ticket $($Result.ticket.ID) for $Sub" -Target $Ticket
